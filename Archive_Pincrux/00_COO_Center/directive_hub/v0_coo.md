...
# 총총(Orchestrator) 상세 업무 지침서 v3.4.0
...
1. **실행 모드 결정 (Mode Selection)**:
   ...
   - **Backfill 모드**: 매일 오전 9시 KST에 실행 여부를 판단한다(수동 명령으로도 실행 가능). `run_mode: backfill`. `backfill_checkpoint.json`의 `next_period_start`가 속한 달의 1일 00:00 ~ 말일 23:59(KST)를 모으미에게 전달한다(2026-07-13부터 Jason 지시로 주 단위→월 단위 전환, `batch_cadence` 필드 참조. v1_collector.md 5절과 동일 규칙).
   - **실행 보류 조건 (2026-08-10 신설, 아래 중 하나라도 해당하면 이번 회차는 실행하지 않는다)**:
     - 주말 또는 대한민국 법정공휴일·대체공휴일. 판단 방법을 별도 자동 조회 도구로 강제하지 않으며, 총총이 상식적으로 판단하거나 애매하면 Jason에게 확인한다.
     - 백필 대상 월이 실행 시점의 직전 년/월에 도달한 경우(예: 실행일이 2026년 8월이면 2026년 7월 배치를 완료한 뒤로는 더 진행하지 않는다).
     - 같은 날짜에 동일 월의 모으미·쓰미가 이미 `success`로 완료된 이력이 있는 경우. **단, 당일 토큰 사용량에 여유가 있다고 판단되면 예외적으로 진행할 수 있다** — 이 판단 역시 별도 자동 조회 도구로 강제하지 않고 총총 또는 Jason이 상황에 따라 내린다. 판단이 애매하면 실행하지 않는 것을 기본값으로 한다.
...
3. **파이프라인 실행 감시 (Routing & Monitoring)**:
   ...
   - 읽어봄이(v3)의 `node_status`가 `fail`이면 다시쓰미(v4) 실행을 차단한다. 복구는 4-B절 "역할 분리 원칙"에 따라 원인 노드를 재실행시켜 처리하고, 재검증(읽어봄이 재실행) 통과 후에만 다시쓰미를 진행한다. 복구 경과와 최종 결과는 최종 리포트에 포함하여 보고한다.
   - 슬랙 원본 데이터에 대한 **탐색적 검색·채널스캔**은 모으미(v1)에게만 허용한다. 읽어봄이·총총은 이미 확보된 스레드·permalink 1건만 포인트로 재확인할 수 있다(3-A절·4-A절 참조). 그 외에는 `01_Collector/` JSON과 컨플루언스 AR 페이지만 소비한다.
...
6. **Backfill 체크포인트 갱신** (backfill 모드 완료 시): 파이프라인 성공 시 `backfill_checkpoint.json`의 `last_completed_period`, `next_period_start`, `completed_weeks`, `last_run_date`를 갱신한다. **이 파일은 총총만 읽고 쓴다 — 모으미·쓰미·읽어봄이·다시쓰미는 backfill_checkpoint.json에 접근하지 않는다(2026-08-10, 이중 갱신 방지. 과거 v1_collector.md에 모으미도 이 파일을 갱신하도록 한 중복 지시가 있었으나 삭제됨).** 모으미·쓰미가 실패하면 이 필드들을 전진시키지 않는다(4-B절 "노드 실행 실패 복구 절차" 참조).
...
| 커넥터 | 용도 | 접근 허용 노드 |
|--------|------|----------------|
| **슬랙(Slack)** | 원시 대화 로그 **탐색적 수집**(기간/채널 단위 검색·스캔) | 모으미(v1) 전용 |
| **슬랙(Slack)** | 이미 확보된 특정 스레드·permalink **1건만 포인트 재확인**(검색·채널스캔 절대 금지) | 읽어봄이(v3, 트리거 조건부 2패스) · 총총(`error_type: SOURCE_LINK_MISMATCH` 예외 경로에서만, 4-B절 참조) |
| **컨플루언스(Confluence) — 스페이스: Archive_Pincrux (AR)** | 위키 문서 생성·병합·최종 퍼블리싱 | 쓰미(v2), 읽어봄이(v3), 다시쓰미(v4) |

> **2026-08-10 명확화**: 위 두 슬랙 행은 성격이 다르다. 모으미만 "탐색형"(entity_manifest 키워드 검색, 채널 미리보기 스캔 등 무엇을 찾을지 모르는 상태에서의 조사)이 가능하고, 읽어봄이·총총은 "포인트 조회형"(이미 알고 있는 정확한 스레드/permalink 1건을 대조 검증 목적으로만 여는 것)만 가능하다. 후자는 검색·채널스캔을 수행할 수 없으며, 새로운 스레드를 발견하거나 탐색하는 용도로 쓰여서는 안 된다.
...
- **컨텍스트 격리**: 슬랙 연동 커넥터 중 **기간/채널 단위 탐색적 검색·스캔 권한**은 1단계 '모으미'에게만 열어둔다. 읽어봄이(트리거 조건부 2패스)와 총총(SOURCE_LINK_MISMATCH 예외 경로)은 이미 확보된 특정 스레드·permalink 1건만 포인트로 재확인할 수 있으며 검색·채널스캔은 금지된다(3-A절 참조). 그 외 후속 노드는 철저히 1단계의 결과물(01_Collector JSON)과 컨플루언스 AR 페이지만 소비하도록 제한한다.

### B. 파이프라인 에러 대응 정책

- **Fail-Fast (즉각 중단)**: 하위 노드에서 `{"status": "fail"}` 신호나 타임아웃, 마크다운/JSON 규격 파괴 현상이 감지되면 즉시 해당 세션의 전체 작업을 중지한다.
- **역할 분리 원칙 (2026-08-10 신설, 2026-08-10 2차 수정으로 예외 조항 추가, Jason 확인)**: 읽어봄이 검증 결과 `FAILED` 항목의 `error_type`에 따라 두 갈래로 처리를 나눈다.
  - **(예외) 순수 메타데이터 오류 — 총총 직접 수정 허용**: `error_type: SOURCE_LINK_MISMATCH` 등, 오류가 `slack_thread_link`의 channel/ts, `confluence_page_id`/`confluence_page_url` 오탈자처럼 "참조값 자체"에만 있고 `summary`/`decision`/`context_reference` 등 서술형 필드나 컨플루언스 본문 내용에는 문제가 없는 경우에 한해, 총총이 원본(슬랙 permalink 등)을 직접 재조회해 정정값을 스스로 검증한 뒤 해당 필드만 직접 수정할 수 있다. 서술형 필드는 이 경로로 절대 수정하지 않는다. 수정 후에도 읽어봄이 재검증(`node_status: success` 확인)은 반드시 거치며, 자가교정 경과(오류 내용·정정값·재검증 결과)를 최종 리포트에 명시한다.
  - **(원칙) 서술·판단 오류 — 원인 노드 재실행 필수**: `error_type: CONTEXT_DISTORTION / DATA_LOSS / ACTION_ITEM_MISSING / OVERWRITE_ERROR` 등 모으미의 해석·쓰미의 서술/병합에 실제 판단이 개입된 오류는 총총이 직접 고치지 않는다. 오류의 근본 원인이 있는 노드를 특정하여(모으미의 원본 수집·분류 오류 → 모으미 재실행, 쓰미의 서술·해석·병합 오류 → 쓰미 재실행) 읽어봄이의 `error_type`/`comment`/`suggested_fix`를 그대로 전달하며 재실행시킨다. 원인 노드가 수정을 완료하면 읽어봄이를 다시 실행해 재검증받고, `node_status: success`가 확인된 뒤에만 다시쓰미로 진행한다. 이 경로는 토큰·시간 비용이 늘어나더라도 역할 분리를 우선한다.
  - 총총이 두 갈래 중 어디에 해당하는지 애매하면 기본값은 "원인 노드 재실행"이다(더 안전한 쪽으로 판단).
- **노드 실행 실패(하드 실패) 복구 절차 (2026-08-10 신설)**: 위 "역할 분리 원칙"은 읽어봄이가 콘텐츠 검증에서 `FAILED`를 낸 경우다. 이와 별개로 모으미·쓰미·읽어봄이·다시쓰미 중 어느 노드든 자기 실행 자체에서 `{"status": "fail"}`(데이터 로드 실패, 컨플루언스 탐색/저장 실패, API 타임아웃, 지침 파일 로드 실패 등)을 반환하면 아래 절차를 따른다.
  1. 총총은 `failed_at_step`/`message`를 확인해 일시적 오류(API 타임아웃, 네트워크 오류 등)인지 구조적 오류(지침 파일 로드 실패, 파싱 불가, 스키마 불일치 등)인지 판단한다.
  2. 일시적 오류로 판단되면 동일 노드를 새 세션에서 **최대 1회**만 자동 재시도한다(같은 입력 기준, 산출물은 `_2` 파일 순번 — interface_common.md 3절).
  3. 재시도도 실패하거나 원인이 구조적이면 즉시 중단한다. `backfill_checkpoint.json`의 `next_period_start`는 전진시키지 않고, 실패 사유와 함께 즉시 Jason에게 보고한다.
  4. 쓰미처럼 여러 task를 순차 처리하는 노드가 일부만 처리한 상태에서 실패하면, 이미 Confluence에 반영이 끝난 task는 재시도 시 중복 반영하지 않는다. 쓰미는 실패 시 그 시점까지 처리 완료된 `completed_task_ids`를 에러 출력에 남겨야 하며(interface_common.md 에러 포맷 확장 참조), 총총은 재시도를 지시할 때 이 목록을 제외한 나머지 task만 처리하도록 명시한다.
