아, 제가 핵심을 완전히 놓쳤군요! 답답하셨을 텐데 바로잡아 주셔서 감사합니다.

모으미가 기껏 [광고주/매체/인하우스]와 [프로젝트/프로덕트]로 정성껏 분류해 놨으니, 쓰미가 그 '분류 체계 자체'를 위키 문서의 뼈대(구조)로 그대로 가져와서 꽂아 넣도록 지침을 전면 수정했습니다.

단순히 날짜순으로 이어 붙이는 게 아니라, **하나의 파일 안에서도 모으미의 분류 기준에 따라 섹션이 나뉘어 누적되도록** 로직을 묶었습니다.

---

### [지침 파일] 02_Writer/instruction.md
...
1. **분류 정합성 유지**: 모으미가 지정한 `major_category`와 `minor_category`를 쓰미가 임의로 변경하거나 재해석하지 않는다.
2. **문맥식 변환**: 슬랙의 구어체("~인 것 같아요", "~했음")를 완벽한 비즈니스 문어체("~로 확인됨", "~하기로 결정함")로 리라이팅하여 기록한다.
3. **미분류(Uncategorized) 배제**: `uncategorized`로 넘어온 데이터는 타겟 문서에 섞지 않고, 무조건 `00_COO_Center/uncategorized_hub.md`로 분리 적치한다.
