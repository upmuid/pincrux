...
- **자체 트래픽/인벤토리를 보유한 대형 사업자**(카드사·통신사·포털 등)는 (a) 자사 서비스 홍보를 위해 핀크럭스에 비용을 지불하는 광고주로 등장할 수도 있고, (b) 자사 앱/플랫폼에 핀크럭스 광고 인벤토리(오퍼월 등)를 태워주는 매체로 등장할 수도 있다. 예: SK플래닛(광고주로 등록되어 있으나 자체 커머스 플랫폼 성격도 있어 재확인 필요), 시럽월렛(SK플래닛 산하 서비스로 추정되나 매체 역할로 등장 — human_action_required로 보류 중).
  - **[정정, 2026-07-24]**: 이전 버전에 "KT(엠모바일=매체로 등장, 프로젝트K/혜택플랫폼=제휴 구조가 매체에 가까움)"를 이중역할 예시로 들었던 것은 오류였다. 'KT 엠모바일'은 KT와 동일 법인이 아니라 별개의 알뜰폰(MVNO) 회사이며, 이는 "하나의 엔티티가 여러 역할을 갖는" 이중역할 사례가 아니라 "브랜드명이 비슷해 보이는 별개 법인" 사례다. 아래 4-1절 참조.

### 4-1. 브랜드명 유사·별개 법인 주의 (이중 역할과 구분할 것)
이중 역할(하나의 엔티티가 문맥에 따라 Client/Media_Partner를 오가는 경우)과, 브랜드명이 비슷해서 같은 회사로 오인하기 쉬운 별개 법인은 서로 다른 문제다. 후자는 major_category를 재판단할 사안이 아니라 **엔티티를 절대 병합하지 않는 것**이 핵심이다.
- **KT vs KT 엠모바일**: 'KT'(196575233)는 KT 통신사가 직접 운영하는 KT클립(CLIP) 앱·혜택플랫폼 라인. 'KT 엠모바일'(192151605)은 KT와 브랜드상 연관은 있으나 별개의 알뜰폰(MVNO) 법인. 2026-07-24 Jason 확인으로 명확화, 과거 KT CLIP 버그 섹션이 KT 엠모바일 페이지에 잘못 병합되어 있던 것을 정정·이관 완료(`entity_history.md` KT 항목 참조).
- **카카오페이 vs 카카오페이지**: 이미 확정된 선행 사례. 카카오페이(kakao pay, 결제 서비스, 192446504)와 카카오페이지(kakaopage, 콘텐츠 플랫폼, 페이지 미생성)는 별개 서비스(2026-07-22 Jason 확인).
